...
# this file reads the txt documents (one per session I think) and
...
members = "(?:" + '|'.join([re.escape(i) for i in set(ft.iloc[:,0])]) + ")"
parties = "(?:" + '|'.join([re.escape(i) for i in set(ft.iloc[:,1])]) + ")"
...
header_format1 = "^\s*(" + members + ")\s*\(" + parties + "\)\s*(?=:)"
...
rules = [
         # match time
         Rule("(?:(?:(?:man|tirs|ons|tors|fre|lør|søn)dag den\s+)?\d{1,2}\.\s+[A-Za-z]+\s+\d{4}\s+)?"\
              "(((kl(\.)?)\s*?)"\
              "([012]?\d[\.:-]?\d{2})([\.:-][012]\d|(?=[^\.:-]+?)|$))", "Time"),
...
# for filename in ["data/txt/20111_M14_helemoedet.txt"]:
    ...
    data.to_csv(filename.replace("/txt/", "/segmented/"), sep = ";", index = False)
